...
def sonic_signal(cHigh, cLow, cClose, cloud_filter):
	...
	if rising_dragon and farer_high != 0 and cLow[-1] >= ema34_high[-1]:
		return ['🟢', atr_per, f"{int(dit_to_high)} atr's to farer high on {farer_high}, ch:{closer_high}"]
	
	elif falling_dragon and  dit_to_low != 0 and cHigh[-1] <= ema34_low[-1]:
		return ['🔴', atr_per, f"{int(dit_to_low)} atr's to farer low on {farer_low}, cl:{closer_low}"]
		
	elif cloud_above == 0 or cloud_below == 0:
		return ['☁️', atr_per, f'{int((abs(cClose[-1] - ema34_basis[-1]) / (cClose[-1] / 100)) / atr_per)} ATRs from Close to Dragon']
