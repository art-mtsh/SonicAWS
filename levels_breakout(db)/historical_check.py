...
def search(filtered_symbols, shared_queue):
    for pair_path in filtered_symbols:
        ...
        highs = {}
        for a in range(1440, len(c_open) - 1440):
            ...
            if a < next_trade_index:
                ...
                max_gap = 0
                for curr_index in range(a - 30, a):
                ...
                if c_high[a] >= max(c_high[a-time_gap:a+time_gap]) and max_gap <= 0.03:

                    for b in range(a+time_gap, a+1440):

                        if c_high[a] == c_high[b] and c_high[a] >= max(c_high[a-time_gap: b+time_gap]):

                            candle_time_a = c_timestamp[a] / 1000
                            candle_time_a = datetime.fromtimestamp(candle_time_a)
                            candle_time_a = candle_time_a.strftime("%d.%m.%Y %H:%M:%S")

                            candle_time_b = c_timestamp[b] / 1000
                            candle_time_b = datetime.fromtimestamp(candle_time_b)
                            candle_time_b = candle_time_b.strftime("%d.%m.%Y %H:%M:%S")

                            print(f"{symbol} {candle_time_a} - {candle_time_b} {c_high[b]}")
                            next_trade_index = b + time_gap
                            break

                            # for c in range(b+time_gap, a+1440):
                            #
                            #     if c_high[a] == c_high[b] == c_high[c] and c_high[a] >= max(c_high[a - time_gap: c + time_gap]):
                            #
                            #         candle_time_c = c_timestamp[c] / 1000
                            #         candle_time_c = datetime.fromtimestamp(candle_time_c)
                            #         candle_time_c = candle_time_c.strftime("%d.%m.%Y %H:%M:%S")
                            #
                            #         print(f"{symbol} {candle_time_a} - {candle_time_b} - {candle_time_c} {c_high[b]}")
                            #         next_trade_index = c + time_gap
                            #         break
                            # break
