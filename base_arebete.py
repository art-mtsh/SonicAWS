...
def calculation(instr, ticksize_filter):
	for frame in range(0, 1):
		for symbol in instr:
			# --- BINANCE DATA ---
			binance_klines = 'https://fapi.binance.com/fapi/v1/klines?symbol=' + symbol + '&interval=' + binance_frame[frame] + '&limit=61'
			...
			if not binance_pd.empty:
				...
				# --- BYBIT DATA ---
				bybit_klines = f'https://api.bybit.com/v5/market/kline?category=inverse&symbol={symbol}&interval={bybit_frame[frame]}&limit=61'
				...
				if not bybit_pd.empty:
					...
					# ==== bybit ticksize ====
					all_ticks = list(bybOpen[-1:-1 - 60:-1]) + \
				            list(bybHigh[-1:-1 - 60:-1]) + \
				            list(bybLow[-1:-1 - 60:-1]) + \
				            list(bybClose[-1:-1 - 60:-1])
