...
from spot_ticksize_dictionary import ticksize_dictionary
...
def calculating(filtered_pairs_ready_to_trade):
    ...
    for key, value in filtered_pairs_ready_to_trade.items():
        if binance_prices and bybit_prices:
            ...
            if binance_bid != 0 and binance_ask != 0 and bybit_bid != 0 and bybit_ask != 0:
                ...
                # print(f"{key}, {'{:.2f}'.format((binance_bid - bybit_ask) / (binance_bid / 100))}%, {'{:.2f}'.format((bybit_bid - binance_ask) / (bybit_bid / 100))}%")
