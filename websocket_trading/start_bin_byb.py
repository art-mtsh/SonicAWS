...
symbols = ["TRBUSDT", "PHBUSDT", "ARKUSDT", "BLZUSDT", "FRONTUSDT"]
...
def on_message_bybit(ws, message):
	...
	data = data.get('data')
	symbol = data.get('s')
	bids = data.get('b')[0][0] if len(data.get('b')) > 0 else 0 # найвища ціна
	asks = data.get('a')[0][0] if len(data.get('a')) > 0 else 0 # найнижча ціна
	
	# print(f"Bybit {symbol} ask {asks}, bid {bids}")
	message_queue.put(("Bybit", symbol, bids, asks))
...
def on_open_bybit(ws):
	...
	subscribe_message = {
		"req_id": "test9292929",
		"op": "subscribe",
		"args": [f"orderbook.1.{symbols[0]}",
		         f"orderbook.1.{symbols[1]}",
		         f"orderbook.1.{symbols[2]}",
		         f"orderbook.1.{symbols[3]}",
		         f"orderbook.1.{symbols[4]}",]
	}
