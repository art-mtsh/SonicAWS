...
def three_distances(symbol, close, combined_list, max_avg_size, search_distance, market_type: str):
	...
	print(f"{market_type}: {symbol} ---> {size_3_dollars} ---> {size_2_dollars} ---> {size_1_dollars}. VOLUME60 {avg_vol_60}")
	
	if size_1 >= max_avg_size and size_1_dollars >= avg_vol_60 / 2 and distance_1 <= search_distance and (price_1 >= max_of_range or min_of_range >= price_1):
		# and max_of_range >= price_1 >= min_of_range:
		
		res.append([distance_1, price_1, size_1])
	
	if size_2 >= max_avg_size and size_2_dollars >= avg_vol_60 / 2 and distance_2 <= search_distance and (price_2 >= max_of_range or min_of_range >= price_2):
		# and max_of_range >= price_2 >= min_of_range:

		res.append([distance_2, price_2, size_2])
	
	if size_3 >= max_avg_size and size_3_dollars >= avg_vol_60 / 2 and distance_3 <= search_distance and (price_3 >= max_of_range or min_of_range >= price_3):
