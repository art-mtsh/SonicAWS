...
def three_distances(symbol, close, combined_list, max_avg_size, search_distance, market_type: str):
	...
	for i in range(2, len(high)):
		if high[-i] == max(high[-1: -i - 5: -1]) and high[-1: -i - 5: -1].count(high[-i]) >= 4 and market_type == "f":
			...
			if distance_4 <= 0.4:
				...
				res.append([distance_4, high[-i], 1000000, 1])
				break

		if low[-i] == min(low[-1: -i - 5: -1]) and low[-1: -i - 5: -1].count(low[-i]) >= 4 and market_type == "f":
			...
			if distance_4 <= 0.4:
				...
				res.append([distance_4, low[-i], 1000000, 1])
				break
