...
def three_distances(symbol, close, combined_list, max_avg_size, search_distance, market_type: str):
	...
	if size_1 >= max_avg_size and size_1_dollars >= avg_vol_60 and distance_1 <= search_distance:
		...
		for i in range(2, len(high)-6):
			if high[-i] >= max(high[-1:-i-5:-1]) and high[-1:-i-5:-1].count(high[-i]) >= 2 and price_1 * 0.999 <= high[-i] <= price_1:
		...
		for i in range(2, len(low)-6):
			if low[-i] <= min(low[-1:-i-5:-1]) and low[-1:-i-5:-1].count(low[-i]) >= 2 and price_1 <= low[-i] <= price_1 * 1.001:
	...
	if size_2 >= max_avg_size and size_2_dollars >= avg_vol_60 and distance_2 <= search_distance:
		...
		for i in range(2, len(high)-6):
			if high[-i] >= max(high[-1:-i - 5:-1]) and high[-1:-i-5:-1].count(high[-i]) >= 2 and price_2 * 0.999 <= high[-i] <= price_2:
		...
		for i in range(2, len(low)-6):
			if low[-i] <= min(low[-1:-i - 5:-1]) and low[-1:-i-5:-1].count(low[-i]) >= 2 and price_2 <= low[-i] <= price_2 * 1.001:
	...
	if size_3 >= max_avg_size and size_3_dollars >= avg_vol_60 and distance_3 <= search_distance:
		...
		for i in range(2, len(high)-6):
			if high[-i] >= max(high[-1:-i - 5:-1]) and high[-1:-i-5:-1].count(high[-i]) >= 2 and price_3 * 0.999 <= high[-i] <= price_3:
		...
		for i in range(2, len(low)-6):
			if low[-i] <= min(low[-1:-i - 5:-1]) and low[-1:-i-5:-1].count(low[-i]) >= 2 and price_3 <= low[-i] <= price_3 * 1.001:
